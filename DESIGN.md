...
- **대화형 `bulti chat`**: grok/opencode처럼 띄워 놓고 다중 턴으로 대화를 이어가는 채팅/TUI 모드. 세션을 저장하고 재개할 수 있다.
- **단발 `bulti run`**: 외부 오케스트레이터(셸 스크립트/CI/다른 에이전트)가 한 번의 호출로 완결시키는 자동화용 단발 모드. 현재 구현을 그대로 유지한다.

두 진입점은 에이전트 루프, 컨텍스트 핸드오프, 스킬/MCP, 히스토리 저장 등 **공통 코어를 공유**하며, 진입점별로 세션 관리·프롬프트 루프·출력 방식만 다르다.
...
2. **세션은 저장하고 작업은 잇는다.** 대화형 모드에서는 세션을 저장·재개한다. 모든 실행은 작업(run) 단위로 기록되고, 컨텍스트가 부족해지면 요약을 새 세그먼트로 넘겨 작업을 자동으로 잇는다(자동 컨텍스트 핸드오프). 단발 모드는 세션 없이 한 번의 호출로 완결되며, 이전 맥락이 필요하면 모델이 history 도구로 조회한다.
...
| 6 | 대화형 세션 저장·재개 + 단발 자동화 | `bulti chat` 세션 저장/재개 + `bulti run` 단발 (§4.12) |
...
| 12 | 대화형 인터페이스 | 다중 턴 채팅/TUI `bulti chat` + 세션 저장·재개 (§4.13) |

### 1.4 비-목표

- **원격 API 프로바이더.** Anthropic, OpenAI, Google 등 클라우드 API를 쓰지 않는다. 로컬 OpenAI 호환 서버만 등록할 수 있다.
- **데몬·서버·API 모드.** 상시 구동되는 프로세스 없이 커맨드 호출식 실행만 한다. 대화형 `bulti chat`도 사용자가 명시적으로 종료할 때까지의 세션일 뿐, 백그라운드 데몬이 아니다.
...
### 2.2 실행 모델: 진입점 × (run → 체인 → 세그먼트)

불티는 두 진입점을 공통 코어 위에 올린다.

- **단발 `bulti run`**: 한 번의 호출로 완결되는 자동화용 실행.
- **대화형 `bulti chat`**: 다중 턴 대화를 이어가며 세션을 저장·재개하는 채팅/TUI 실행.

두 진입점 모두 아래의 실행 단위를 공유한다.

- **run**: `bulti run` 한 번의 호출, 또는 `bulti chat`에서 한 턴(사용자 프롬프트 하나 → 모델 완료 응답)을 말한다. 사용자에게는 하나의 작업 단위로 보인다.
- **세그먼트(segment)**: 하나의 메시지 히스토리(시스템 프롬프트 + 프롬프트 + 도구 결과)로 완결되는 에이전트 루프 단위. 단발 모드에서는 세그먼트가 곧 세션이며, 대화형 모드에서는 한 턴이 하나 이상의 세그먼트로 실행될 수 있다.
...
- **세션(session)**: 대화형 모드 전용. 사용자 턴들의 연속을 세션(`session_id`)으로 묶어 저장한다. 세션은 여러 run(턴)을 포함하며, 재개 시 이전 대화 맥락을 프롬프트에 복원한다 (§4.13).
...
CREATE TABLE runs (
  ...
  session_id    TEXT,              -- 대화형 모드면 세션 UUID (§4.13), 단발이면 NULL
...
- **모델 도구 (레이지 정신의 확장)**: `history_list(query?, limit?)`와 `history_read(run_id)`. 단발 모드에서는 "이전 작업 이어서" 같은 지시가 모델이 이 도구들로 스스로 맥락을 회수하게 만든다. 대화형 모드에서는 세션 재개(§4.13)가 맥락을 복원한다. 시스템 프롬프트에는 도구 존재만 한 줄로 안내한다.
...
bulti chat [옵션]                # 대화형 채팅/TUI (§4.13)
  [--session ID] [--resume] [--endpoint NAME] [--model M]
  [--no-tui]                    # TUI 대신 스트림 텍스트로 대화
...
- **승인 프롬프트 없음**: 외부 오케스트레이션 전제이므로 도구 실행 승인을 기다리지 않는다. 파괴 명령 완화는 §6의 정책 설정으로 제공한다. (대화형 `bulti chat`에서는 §4.13의 승인 정책을 별도로 적용할 수 있다.)
- 예제 스크립트를 `examples/` 디렉터리에 둔다 (셸 파이프라인, CI 잡, 다른 에이전트의 subprocess 호출).

### 4.13 대화형 인터페이스 (`bulti chat`)

요구사항 12. grok/opencode처럼 띄워 놓고 다중 턴으로 대화를 이어가는 채팅/TUI 모드다. 단발 `bulti run`과 공통 코어(에이전트 루프, 컨텍스트 핸드오프, 스킬/MCP, 히스토리 저장)를 공유한다.

#### 4.13.1 진입점과 프롬프트 루프

- `bulti chat`은 TTY에서 대화형 프롬프트 루프를 시작한다. `--no-tui`면 ratatui 대신 스트림 텍스트로 대화한다.
- 한 턴(사용자 프롬프트 입력 → 모델 완료 응답)은 `agent::session`이 주관하며, 내부적으로 §4.3의 에이전트 루프를 세그먼트 단위로 실행한다. 턴이 컨텍스트 한계에 도달하면 §4.6 핸드오프로 이어 간다.
- 프롬프트 루프는 `Ctrl-D`/`/exit`로 종료, `/resume <id>`로 세션 재개, `/new`로 새 세션 시작을 지원한다.

#### 4.13.2 세션 저장·재개

- 세션은 `~/.bulti/sessions/<session_id>.json`에 저장한다 (§3). 세션은 사용자 턴들의 연속을 담는다.
- **저장**: 매 턴 종료 시 세션 파일에 메시지 배열을 갱신한다. 세션에는 시스템 프롬프트 조합 시각, 턴별 사용자·모델 메시지, 도구 결과 요약, 파일 경로·결정사항을 포함한다.
- **재개**: `bulti chat --resume <id>`(또는 `/resume`) 시 세션 파일의 메시지 배열을 프롬프트에 복원한다. 컨텍스트가 부족하면 §4.6 핸드오프 요약으로 압축해 이어 간다.
- 세션은 히스토리 DB의 `session_id` 필드로도 연결된다 (§4.7). 세션 파일은 대화형 재개용 원본, 히스토리 DB는 작업 단위 감사용 기록으로 분리한다.

#### 4.13.3 TUI 렌더링

- `src/tui/`(ratatui + crossterm)이 대화·도구 호출·결과를 화면에 렌더링한다.
- 단발 `run`의 라이브 출력 규약(🔧 툴 호출, 두 칸 들여쓰기 결과, 💭 reasoning)을 그대로 재사용하되, TUI 패널로 배치한다.
- 스크롤백, 입력 라인, 상태 표시(엔드포인트·모델·토큰·세션 id)를 제공한다.

#### 4.13.4 승인 정책

- 단발 모드와 달리 대화형 모드는 기본적으로 도구 실행 전 사용자 승인을 받을 수 있다. `[tools] approve = "prompt" | "auto"` 설정으로 제어한다(기본 `auto` — 단발 모드와 동일하게 자율 실행).
- `approve = "prompt"`면 파괴성 도구(write_file/edit_file/bash) 실행 직전에 `[y/N]` 확인을 받는다. 이는 대화형 사용자가 직접 눈으로 확인하며 작업을 진행하는 용도에 맞춘다.
...
상세 태스크는 shepherd 이슈(bulti 프로젝트)로 관리한다. 단발 기능은 13개 단계 이슈(#101~#112 부모 아래)가 순서대로 연결되고, 대화형 인터페이스는 별도 부모 이슈(#114) 아래 6개 단계 이슈(#115~#120)가 연결된다.
...
**대화형 인터페이스 단계 (부모 이슈 #114 — 단발 기능 위에 추가):**

| 단계 | 이슈 | 범위 | 의존 |
|---|---|---|---|
| 1 | 설계 문서 개편 (대화형 인터페이스 도입) | DESIGN.md 전반 | — |
| 2 | 대화형 진입점과 프롬프트 루프 | §4.13.1 | 단발 5(핸드오프) |
| 3 | 세션 저장·재개 | §4.13.2 | 대화형 2 |
| 4 | 대화형 모드에서 코어 기능 연동 | §4.13 (스킬/MCP/히스토리 공유) | 대화형 3 |
| 5 | TUI 채팅 인터페이스 | §4.13.3 | 대화형 4 |
| 6 | 통합 검증과 릴리즈 | 대화형·단발 e2e, 문서, 릴리즈 | 대화형 5 |
...
- **run**: `bulti run` 호출 한 번, 또는 `bulti chat`의 한 턴. 사용자 관점 작업 단위.
- **세그먼트**: 하나의 메시지 히스토리로 완결되는 루프 단위. 세션의 기본 실행 단위.
- **체인**: 핸드오프로 이어진 세그먼트들의 묶음 (`chain_id`).
- **세션**: 대화형 모드에서 사용자 턴들의 연속 (`session_id`). 저장·재개 가능 (§4.13).
